...
![152850207a63984082](/Users/jack/Documents/learn/K8s/images/152850207a63984082.png)
...
- 容器的自动调度，我们也会把它叫做 schedule，就是“调度”，把一个容器放到一个集群的某一个机器上，Kubernetes 会帮助我们去做存储的编排，让存储的声明周期与容器的生命周期能有一个连接；

- Kubernetes 会帮助我们去做自动化的容器的恢复。在一个集群中，经常会出现宿主机的问题或者说是 OS 的问题，导致容器本身的不可用，Kubernetes 会自动地对这些不可用的容器进行重建；
...
Kubernetes 可以把用户提交的创建容器的请求放到 Kubernetes 集群的某一台管理节点上去。Kubernetes 的调度器会观察需要被调度的这个容器的需求。比如说它所需要的 CPU以及它所需要的 memory，然后在集群中找一台相对合适的机器来进行一次 placement，也就是一次放置的操作。
...
Kubernetes 的 Node 是真正运行业务负载的，每个业务负载会以 Pod 的形式运行。等一下我会介绍一下 Pod 的概念。一个 Pod 中运行的一个或者多个容器，真正去运行这些 Pod 的组件的是叫做 **kubelet**，也就是 Node 上最为关键的组件，它通过 API Server 接收到所需要 Pod 运行的状态，然后提交到我们下面画的这个 Container Runtime 组件中。
 ...
 ![node arch](/Users/jack/Documents/learn/K8s/images/153559f9addd084872.png)
...
![schedule](/Users/jack/Documents/learn/K8s/images/1536342d14bf291564.png)
...
![url](/Users/jack/Documents/learn/K8s/images/154014e44d84573629.png)
